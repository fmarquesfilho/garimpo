...
import { payloadToConfig, gerarResumo, contarFiltrosAtivos, gerarLabelBusca, cronLabel } from './busca-unificada-logic.js';
import { DEFAULTS, normalizarComissao, normalizarVendas, intentBusca, proximoModo, buscarDuplicada, BUSCA_DUPLICADA, MARKETPLACES } from './busca-config.js';
import { criarContextoInicial, criarUIInicial, guards, STATES, MODOS } from './busca-engine-state.js';
import { processarOmniboxInput, processarOmniboxKeydown, resolverOpcaoAtiva, classificarIntencao, classificarSugestaoPrefixo } from './busca-engine-omnibox.js';
...
export class BuscaEngine {
	...
	// ── Derivados ─────────────────────────────────────────────────────────
	get loading() { return this.status === STATES.SEARCHING; }
	get resumo() { return gerarResumo(this.ctx); }
	get filtrosAtivos() { return contarFiltrosAtivos(this.ctx); }
	get fontesAtivas() { return Object.entries(this.ctx.fontes).filter(([, v]) => v).map(([k]) => k); }
	get intent() { return intentBusca(this.ctx); }
	...
	get modo() { return this.ctx.modo; }
	get contadorLojas() { return this.ctx.shopIds.length; }
	get contadorBuscas() { return this.ctx.buscasSalvas.length; }
	get buscaDuplicada() {
	...
	get lojaCards() {
		return this.ctx.shopIds.map((id) => {
			...
			return { id, nome: this.ctx.shopNomes[id] || id, ...meta, tipo: meta.tipo ?? (meta.cron ? 'monitorada' : 'escopada') };
		});
	}
	get contadorFiltros() {
		...
		for (const [k, v] of Object.entries(this.ctx.fontes)) { if (v !== DEFAULTS.fontes[k]) n++; }
		return n;
	}
	...
	#mudarFontes(event) { this.ctx.fontes = event.fontes; this.#debounce(); }
	#mudarMarketplaces(event) { this.ctx.marketplacesFiltro = event.marketplaces ?? []; this.#debounce(); }
	...
	#adicionarCategoria(event) {
		...
		if (event.categoria?.marketplaces) this.ctx.categoriaMeta = { ...this.ctx.categoriaMeta, [nome]: event.categoria.marketplaces };
		this.#executarBusca();
	}

	#removerCategoria(event) {
		this.ctx.categorias = this.ctx.categorias.filter((c) => c !== event.nome);
		const meta = { ...this.ctx.categoriaMeta }; delete meta[event.nome]; this.ctx.categoriaMeta = meta;
		this.#debounce();
	}
	...
	async #resolverLojaRemota(input) {
		...
		try {
			...
			this.ctx.resolucaoLoja = { status: 'erro', erro: abortou ? 'Timeout na resolucao da loja (10s).' : (e?.message ?? 'Falha ao resolver loja') };
	...
	async #salvar() {
		...
		if (!validacao.ok) { this.ctx.erroDuplicata = validacao.erro ?? null; return; }
		...
		try {
			...
			this.ctx.editandoId = null; this.ctx.buscaSelecionadaId = null;
			this.ctx.modo = MODOS.EXPLORANDO; this.salvarAberto = false;
			this.status = STATES.RESULTS;
		} catch (e) { this.ctx.error = e?.message ?? 'Falha ao salvar'; this.status = STATES.RESULTS; }
	}

	#carregarSalva(event) {
		Object.assign(this.ctx, restaurarConfig(event.config));
		this.ctx.buscaSelecionadaId = event.config.id ?? null; this.ctx.editandoId = null;
		this.#executarBusca();
	}
	...
	#editarSalva(event) {
		...
		this.salvarAberto = true; this.#executarBusca();
	}

	#cancelarEdicao() { this.ctx.editandoId = null; this.ctx.buscaSelecionadaId = null; this.salvarAberto = false; }
	...
	get #intencaoCtx() { return { categoriasDisponiveis: this.ctx.categoriasDisponiveis, marketplacesFiltro: this.ctx.marketplacesFiltro, shopIds: this.ctx.shopIds, shopNomes: this.ctx.shopNomes }; }
	get #sugestoesCtx() { return { lojasMonitoradas: this.ctx.lojasDisponiveis, categoriasDisponiveis: this.ctx.categoriasDisponiveis, marketplaces: MARKETPLACES?.suportados ?? [], buscasSalvas: this.ctx.buscasSalvas }; }

	#omniboxInput(event) {
		const r = processarOmniboxInput(event.value ?? '', this.#intencaoCtx, this.#sugestoesCtx);
		Object.assign(this.ui.omnibox, { inputValue: r.inputValue, aberto: r.aberto, highlightIdx: r.highlightIdx, modo: r.modo, opcoes: r.opcoes });
		this.ctx.keyword = r.keyword; this.#debounce();
	}

	#omniboxKeydown(event) {
		const r = processarOmniboxKeydown(event.key, event.idx, this.ui.omnibox.highlightIdx, this.ui.omnibox.opcoes.length);
	...
	#omniboxSelecionar(event) {
		...
		if (event.indice >= 0 && event.indice < opcoes.length) {
			this.ui.omnibox.aberto = false; this.ui.omnibox.highlightIdx = -1;
			this.#executarOpcao(opcoes[event.indice]);
		}
	}

	#omniboxBlur() { this.ui.omnibox.aberto = false; this.ui.omnibox.highlightIdx = -1; }
	...
	#executarOpcao(opcao) {
		if (this.ui.omnibox.modo === 'intencao') {
			...
			if (action === 'BUSCAR_PRODUTOS') { this.ctx.keyword = payload.keyword; this.ui.resultados.modo = 'produtos'; this.#executarBusca(); }
			else if (action === 'BUSCAR_LOJAS') { this.#buscarLojas(payload); }
			else if (action === 'ADICIONAR_CATEGORIA') { this.#adicionarCategoria({ nome: payload.nome }); this.ctx.keyword = ''; this.ui.omnibox.inputValue = ''; this.#executarBusca(); }
			else if (action === 'RESOLVER_LINK') { this.#adicionarLoja({ value: payload.url }); this.ui.resultados.modo = 'lojas'; }
		} else {
			...
			else if (action === 'ADICIONAR_CATEGORIA') this.send({ type: 'ADICIONAR_CATEGORIA', nome: payload.nome, categoria: payload.categoria });
			else if (action === 'MUDAR_MARKETPLACES') this.send({ type: 'MUDAR_MARKETPLACES', marketplaces: [...new Set([...this.ctx.marketplacesFiltro, payload.marketplace])] });
	...
	async #buscarLojas(event) {
		...
		this.ui.resultados.modo = 'lojas'; this.status = STATES.SEARCHING;
		try {
			const r = await this.#effects.buscarLojasPorNome(termo);
			this.ui.resultados.lojas = r?.lojas ?? []; this.status = STATES.RESULTS;
		} catch (e) { this.ctx.error = e?.message ?? 'Falha ao buscar lojas'; this.ui.resultados.lojas = []; this.status = STATES.ERROR; }
	}
	...
	async #monitorarLoja(event) {
		...
		try {
			...
			this.ui.resultados.lojas = this.ui.resultados.lojas.map((l) => String(l.id) === String(event.loja.id) ? { ...l, monitorada: true } : l);
			this.ctx.resolucaoLoja = { status: 'idle' };
		} catch (e) { this.ctx.resolucaoLoja = { status: 'erro', erro: e?.message ?? 'Falha ao monitorar loja' }; }
	}
	...
	#debounce() {
		...
		this.#debounceTimer = setTimeout(() => {
			if (guards.temContextoBusca(this.ctx)) { this.#executarBusca(); }
			else { this.ctx.resultados = []; this.ctx.contagens = { curadoria: 0, quedas: 0, novos: 0, lojas: 0 }; this.status = STATES.IDLE; }
		}, DEFAULTS.debounceMs);
	}

	async #executarBusca() {
		this.status = STATES.SEARCHING; this.ctx.error = null;
		try {
			this.ctx.dadosBrutos = await this.#effects.executarBusca(this.ctx);
			this.#refiltrar(); this.status = STATES.RESULTS;
		} catch (e) { this.ctx.error = e?.message ?? 'A busca falhou'; this.status = STATES.ERROR; }
	}

	#refiltrar() {
		const r = montarResultados({ fontes: this.ctx.fontes, dadosCuradoria: this.ctx.dadosBrutos.curadoria ?? [], dadosQuedas: this.ctx.dadosBrutos.quedas ?? [], dadosNovos: this.ctx.dadosBrutos.novos ?? [], dadosLojas: this.ctx.dadosBrutos.lojas ?? [], favoritos: this.ctx.dadosBrutos.favoritos ?? [], busca: this.ctx.keyword, categorias: this.ctx.categorias, comissaoMin: this.ctx.comissaoMin, vendasMin: this.ctx.vendasMin });
		this.ctx.resultados = r;
		this.ctx.contagens = { curadoria: r.filter((p) => p._fonte === 'curadoria').length, quedas: r.filter((p) => p._fonte === 'queda').length, novos: r.filter((p) => p._fonte === 'novo').length, lojas: r.filter((p) => p._fonte === 'loja').length };
	}
}
