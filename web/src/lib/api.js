 ...
 */
async function postar(caminho, corpo, { signal } = {}) {
 ...
 * @param {{ signal?: AbortSignal }} [http] permite abortar no timeout
 */
export function resolverLoja({ input, marketplace, origem }, { signal } = {}) {
	const corpo = { input };
	if (marketplace) corpo.marketplace = marketplace;
	if (origem) corpo.origem = origem;
	return postar('/api/lojas/resolver', corpo, { signal });
}
