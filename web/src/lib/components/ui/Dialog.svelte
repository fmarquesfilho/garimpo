<script>
	...
	let { open = $bindable(false), title = '', description = '', children, ...rest } = $props();
</script>
...
<Dialog.Root bind:open {...rest}>
	<Dialog.Portal>
		...
		<Dialog.Content
			class="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-lg border border-border bg-popover p-6 shadow-lg animate-in fade-in-0 zoom-in-95"
		>
			{#if title}
