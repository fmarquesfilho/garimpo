		...
		<div class="corpo">
			...
			<header>
				...
				<div class="meta">
					...
					{#if produto.origem}
						<span class="selo origem">{#if produto.origem === 'Coreia'}🇰🇷{:else if produto.origem === 'Japão'}🇯🇵{:else if produto.origem === 'China'}🇨🇳{/if} {produto.origem}</span>
					{/if}
...
<style>
	/* ═══ FULL LAYOUT ═══ */
	.cartao { background: var(--nevoa); border: 1px solid var(--linha); border-radius: var(--raio); overflow: hidden; box-shadow: var(--sombra); transition: transform 0.15s ease, box-shadow 0.15s ease; }
	.cartao:hover { transform: translateY(-2px); box-shadow: 0 4px 20px -8px rgba(46, 34, 38, 0.2); }
	.destaque { border-color: var(--ouro-claro); }
	.thumb { width: 100%; height: 180px; object-fit: cover; display: block; background: var(--porcelana); }
	...
	.selo { font-size: 0.65rem; font-weight: 700; padding: 1px 6px; border-radius: var(--raio-full); }
	.selo.alerta { background: var(--erro-fundo); color: var(--erro-texto); }
	.selo.origem { background: var(--sucesso-fundo); color: var(--sucesso-texto); }
	.selo.desconto { background: var(--erro-fundo); color: var(--erro-texto); }
	.selo.expira { background: var(--ouro-fundo); color: var(--ouro-escuro); }
	.dados { display: flex; justify-content: space-between; align-items: baseline; }
	.dado-principal { display: flex; align-items: baseline; gap: var(--r2); }
	.preco { font-size: 1.1rem; font-weight: 700; font-family: var(--mono); }
	.comissao { font-size: var(--text-sm); font-weight: 700; color: var(--ouro); }
	.dado-secundario { display: flex; gap: var(--r3); font-size: var(--text-xs); color: var(--tinta-suave); }
	footer { display: flex; gap: var(--r2); margin-top: var(--r2); }
	footer button { border-radius: var(--raio-sm); padding: 8px 14px; font-size: var(--text-sm); font-weight: 600; border: 1px solid transparent; }
	.publicar-btn { background: var(--ouro-fundo); border-color: var(--ouro-claro); color: var(--ouro-escuro); flex: 1; }
	...
	.compact-thumb { width: 56px; height: 56px; border-radius: 8px; object-fit: cover; flex-shrink: 0; }
	.compact-info { flex: 1; min-width: 0; }
	.compact-info h4 { font-size: 0.9rem; margin: 0 0 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
	.compact-dados { display: flex; flex-wrap: wrap; gap: var(--r2); font-size: 0.78rem; color: var(--tinta-suave); }
	.compact-preco { font-weight: 700; color: var(--ouro); }
	.compact-comissao { font-weight: 600; }
	.compact-loja { font-size: 0.72rem; color: var(--tinta-suave); }
	...
	.feed-card:hover { border-color: var(--ouro-claro); }
	.feed-card.queda { border-left: 3px solid var(--sucesso-texto); }
	.feed-card.alta { border-left: 3px solid var(--erro-texto); }
	.feed-card.novo { border-left: 3px solid var(--ouro); }
	.feed-header { display: flex; align-items: center; gap: var(--r2); margin-bottom: 6px; }
	.badge { padding: 2px 8px; border-radius: var(--raio-full); font-size: 0.72rem; font-weight: 700; }
	...
	.badge-novo { background: var(--ouro-fundo); color: var(--ouro-escuro); }
	.feed-loja { font-size: 0.72rem; color: var(--tinta-suave); background: var(--porcelana); padding: 1px 6px; border-radius: 4px; }
	.feed-tempo { font-size: 0.72rem; color: var(--tinta-suave); margin-left: auto; }
	...
	.feed-comissao { font-size: 0.78rem; color: var(--ouro); font-weight: 600; }
	.feed-vendas { font-size: 0.78rem; color: var(--tinta-suave); }
	.feed-acoes { margin-top: 10px; display: flex; gap: var(--r2); }
	.btn-publicar { padding: 6px 14px; border: 1px solid var(--ouro-claro); background: var(--ouro-fundo); color: var(--ouro-escuro); border-radius: var(--raio-sm); font-size: 0.82rem; font-weight: 600; cursor: pointer; }
