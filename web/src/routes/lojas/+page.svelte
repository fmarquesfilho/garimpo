<script>
	...
	import { Tabs, Loading, Alert, PageHeader, EmptyState } from '$lib/components/ui/index.js';
...
<section class="max-w-[900px] space-y-10">
	<PageHeader
		rotulo="monitoramento"
		titulo="Lojas monitoradas"
		subtitulo="Acompanhe os produtos das lojas que você monitora — novidades, variações de preço e publicação de ofertas em um só lugar."
	/>

	{#if !$usuario}
		<EmptyState icone="🔒" mensagem="Faça login para ver as lojas monitoradas." />
	{:else}
		<!-- 1. Suas lojas — conteúdo primário -->
		<div class="space-y-4">
			<div class="flex items-center justify-between">
				<h2 class="m-0 text-lg text-foreground">🏪 Suas lojas</h2>
				{#if buscasComLojas.length}
					<span class="text-sm text-muted-foreground"
						>{buscasComLojas.length}
						{buscasComLojas.length === 1 ? 'loja' : 'lojas'}</span
					>
				{/if}
			</div>
			...
			{#if buscasComLojas.length === 0}
				<EmptyState
					icone="🏪"
					mensagem="Nenhuma loja monitorada ainda."
					dica="Use o formulário “Adicionar loja” abaixo para começar a acompanhar uma loja Shopee."
				/>
			{:else}
				<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
					{#each buscasComLojas as b (b.id)}
						<div class="relative">
							<button
								class="flex h-full w-full cursor-pointer flex-col gap-2 rounded-md border bg-card p-4 pr-9 text-left transition-colors hover:border-ouro {buscaSelecionada?.id ===
								b.id
									? 'border-ouro bg-ouro-fundo ring-1 ring-ouro'
									: 'border-border'}"
								onclick={() => selecionarBusca(b)}
							>
								<span class="flex items-center gap-2">
									<span class="text-base">🏪</span>
									<strong class="text-sm text-foreground">{b.nome || b.id}</strong>
								</span>
								{#if b.keywords?.length}
									<span class="flex flex-wrap gap-1">
										{#each b.keywords.slice(0, 4) as kw (kw)}
											<span class="rounded-full bg-muted px-2 py-0.5 text-[0.7rem] text-muted-foreground">{kw}</span>
										{/each}
										{#if b.keywords.length > 4}
											<span class="text-[0.7rem] text-muted-foreground">+{b.keywords.length - 4}</span>
										{/if}
									</span>
								{:else}
									<span class="text-xs text-muted-foreground">Monitorando todos os produtos</span>
								{/if}
								<span class="mt-auto text-xs text-muted-foreground"
									>{b.cron ? '⏱ coleta automática' : 'coleta manual'}</span
								>
							</button>
							<button
								class="absolute top-2 right-2 flex h-[22px] w-[22px] cursor-pointer items-center justify-center rounded-full border-none bg-transparent text-xs text-muted-foreground hover:bg-erro-fundo hover:text-erro"
				...
				{#if buscaSelecionada}
					<div class="rounded-md border border-border bg-card p-4">
						<p class="mb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
							{buscaSelecionada.nome || buscaSelecionada.id}
						</p>
						<Tabs {tabs} bind:active={aba}>
							...
							{:else if aba === 'novidades'}
								...
								{:else if !novidades || novidades.produtos_novos?.length === 0}
									<EmptyState
										icone="🆕"
										mensagem="Nenhum produto novo detectado nos últimos {novidades?.dias_janela ?? 7} dias."
									/>
								{:else}
							...
							{:else if aba === 'precos'}
								...
								{:else if !novidades || novidades.variacoes?.length === 0}
									<EmptyState
										icone="📉"
										mensagem="Nenhuma variação de preço detectada nos últimos {novidades?.dias_janela ?? 7} dias."
									/>
								{:else}
									<p class="text-sm mb-4">
		...
		<!-- 3. Buscas por palavra-chave (Shopee inteira) -->
		<GerenciarBuscas />
