---
title: Variáveis de ambiente
description: Lista de variáveis de ambiente extraídas do código-fonte.
---

:::caution[Arquivo gerado]
Não edite manualmente. Rode `make docs-env` para regenerar.
:::

| Variável | Arquivo | Linha |
|---|---|---|
| `ALERTAS_APENAS_QUEDAS` | `internal/alerts/alerts.go` | 45 |
| `ALERTAS_TELEGRAM_CHAT_ID` | `internal/alerts/alerts.go` | 56 |
| `ALERTAS_TELEGRAM_TOKEN` | `internal/alerts/alerts.go` | 49 |
| `ALERTAS_THRESHOLD` | `internal/alerts/alerts.go` | 40 |
| `ALERTER_ADDR` | `services/scheduler/main.go` | 36 |
| `BQ_DATASET` | `internal/store/novo_gcp.go` | 15 |
| `BQ_TABELA_SNAP` | `internal/store/novo_gcp.go` | 20 |
| `BQ_TABELA` | `internal/store/novo_gcp.go` | 16 |
| `COLLECTOR_ADDR` | `services/scheduler/main.go` | 28 |
| `ENCRYPTION_KEY` | `internal/crypto/crypto.go` | 26 |
| `GOOGLE_CLOUD_PROJECT` | `internal/store/novo_gcp.go` | 14 |
| `PORT` | `services/alerter/main.go` | 23 |
| `PUBLISHER_ADDR` | `services/scheduler/main.go` | 32 |
| `SHOPEE_APP_ID` | `services/collector/main.go` | 28 |
| `SHOPEE_SECRET` | `services/collector/main.go` | 29 |
| `TELEGRAM_BOT_TOKEN` | `internal/alerts/alerts.go` | 51 |
| `TELEGRAM_CHAT_ID` | `internal/publish/novo.go` | 21 |
| `WHATSAPP_ACCESS_TOKEN` | `internal/publish/novo.go` | 23 |
| `WHATSAPP_PHONE_NUMBER_ID` | `internal/publish/novo.go` | 22 |
