...
| T-0019 Migrar handlers de lojas/buscas para C# (G) | T-0010 Proto definitions + shopee-collector gRPC server (M) | | | T-0008 Refactor: tratamento de erros idiomático + telemetria (G) |
| T-0020 PostgreSQL como fonte primária + BigQuery analytics-only (G) | T-0011 Publisher gRPC server (extract publish package) (M) | | | T-0009 Setup mono-repo (Go + C# + protos) + Docker Compose (M) |
| T-0022 Descomissionar monólito Go (M) | T-0012 Scheduler gRPC server (serviço Go separado) (M) | | | |
