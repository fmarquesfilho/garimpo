...
| T-0020 PostgreSQL como fonte primária + BigQuery analytics-only (G) | T-0014 PostgreSQL schema + EF Core migrations (dados transacionais) (M) | | | T-0009 Setup mono-repo (Go + C# + protos) + Docker Compose (M) |
| T-0022 Descomissionar monólito Go (M) | T-0015 Multi-tenant em C# (EF Core + PostgreSQL) (G) | | | T-0010 Proto definitions + shopee-collector gRPC server (M) |
| | T-0016 Curadoria controller + scoring port em C# (G) | | | T-0011 Publisher gRPC server (extract publish package) (M) |
| | T-0021 Cloud Run multi-container deploy (C# + sidecars Go) (M) | | | T-0012 Scheduler gRPC server (serviço Go separado) (M) |
